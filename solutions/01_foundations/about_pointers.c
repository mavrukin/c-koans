...
KOAN(nullptr_is_the_modern_null)
{
    ...
    int n = 0;
    int *q = &n;
    KOAN_EQ_INT(/*__*/ 0, p ? 1 : 0);
    KOAN_EQ_INT(/*__*/ 1, q ? 1 : 0);
}
...
KOAN(the_star_binds_to_the_name)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, *p);
    KOAN_EQ_INT(/*__*/ 2, q);
    KOAN_EQ_SZ(/*__SZ*/ sizeof(int), sizeof q);
}
...
KOAN(const_position_decides_what_is_protected)
{
    ...
    KOAN_EQ_CHR(/*__CHR*/ 'a', to_const[0]);

    to_const = "xyz";               /* legal: the pointer may move */
    KOAN_EQ_CHR(/*__CHR*/ 'x', to_const[0]);

    fixed[0] = 'z';                 /* legal: the pointee may change */
    KOAN_EQ_STR(/*__STR*/ "zbc", buf);
}
...
KOAN(pointer_arithmetic_steps_by_pointee_size)
{
    ...
    KOAN_EQ_INT(/*__*/ 20, *(ip + 1));

    /* Advancing an int* by 1 advances the address by sizeof(int). */
    KOAN_EQ_SZ(/*__SZ*/ 4, (size_t)((char *)(ip + 1) - (char *)ip));
    KOAN_EQ_SZ(/*__SZ*/ 1, (size_t)((cp + 1) - cp));

    /* Subtracting two pointers into the same array yields an element count. */
    KOAN_EQ_SZ(/*__SZ*/ 3, (size_t)(&nums[3] - &nums[0]));
}
...
KOAN(pointer_identity_is_address_identity)
{
    ...
    const char *s = "hello";
    KOAN_EQ_INT(/*__*/ 0, strcmp(s, "hello"));
}
...
KOAN(assembling_a_linked_list_with_pointer_to_pointer)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, n);
    KOAN_EQ_INT(/*__*/ 1, got[0]);
    KOAN_EQ_INT(/*__*/ 2, got[1]);
    KOAN_EQ_INT(/*__*/ 3, got[2]);

    /* Removing the head needs no special handling at all. */
    KOAN_EQ_INT(/*__*/ 1, remove_value(&head, 1));
    n = list_to_array(head, got, 4);
    KOAN_EQ_SZ(/*__SZ*/ 2, n);
    KOAN_EQ_INT(/*__*/ 2, got[0]);

    /* Nor does removing from the middle, or failing to find anything. */
    KOAN_EQ_INT(/*__*/ 0, remove_value(&head, 99));
    KOAN_EQ_INT(/*__*/ 1, remove_value(&head, 3));
    n = list_to_array(head, got, 4);
    KOAN_EQ_SZ(/*__SZ*/ 1, n);
    KOAN_EQ_INT(/*__*/ 2, got[0]);
}
