...
KOAN(arrays_know_their_own_size_where_declared)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 4, sizeof nums / sizeof nums[0]);
    KOAN_EQ_INT(/*__*/ 30, nums[2]);
}
...
KOAN(short_initialisers_zero_the_rest)
{
    ...
    KOAN_EQ_INT(/*__*/ 2, partial[1]);
    KOAN_EQ_INT(/*__*/ 0, partial[2]);
    KOAN_EQ_INT(/*__*/ 0, partial[4]);

    int all_zero[100] = { 0 };
    int sum = 0;
    for (size_t i = 0; i < 100; i++) sum += all_zero[i];
    KOAN_EQ_INT(/*__*/ 0, sum);

    /* C23 permits an empty initialiser, which reads better than { 0 }. */
    int also_zero[8] = {};
    KOAN_EQ_INT(/*__*/ 0, also_zero[7]);
}
...
KOAN(designated_initialisers_pick_elements)
{
    ...
    KOAN_EQ_INT(/*__*/ 10, sparse[1]);
    KOAN_EQ_INT(/*__*/ 40, sparse[4]);
    KOAN_EQ_INT(/*__*/ 0, sparse[0]);
    KOAN_EQ_INT(/*__*/ 0, sparse[5]);
}
...
KOAN(indexing_is_pointer_arithmetic)
{
    ...
    KOAN_EQ_INT(/*__*/ 6, nums[1]);
    KOAN_EQ_INT(/*__*/ 6, *(nums + 1));
    KOAN_EQ_INT(/*__*/ 6, 1[nums]);
}
...
KOAN(arrays_decay_to_pointers_when_passed)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 40, sizeof nums);
    KOAN_EQ_SZ(/*__SZ*/ sizeof(int *), sizeof_inside(nums));
...
KOAN(the_array_and_its_first_element_share_an_address)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 16, (size_t)((char *)(&nums + 1) - (char *)&nums));
    KOAN_EQ_SZ(/*__SZ*/ 4, (size_t)((char *)(nums + 1) - (char *)nums));
}
...
KOAN(arrays_are_copied_and_compared_by_bytes)
{
    ...
    KOAN_EQ_INT(/*__*/ 3, dst[2]);
    KOAN_EQ_INT(/*__*/ 0, memcmp(src, dst, sizeof src));
...
KOAN(one_past_the_end_may_be_addressed_not_read)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, (size_t)(end - begin));

    int sum = 0;
    for (int *p = begin; p != end; p++) sum += *p;
    KOAN_EQ_INT(/*__*/ 6, sum);
}
...
KOAN(bounds_are_your_responsibility)
{
    ...
    KOAN_EQ_INT(/*__*/ 8, at_or_default(nums, 3, 1, -1));
    KOAN_EQ_INT(/*__*/ -1, at_or_default(nums, 3, 3, -1));
    KOAN_EQ_INT(/*__*/ -1, at_or_default(nums, 3, 99999, -1));
}
...
KOAN(assembling_an_array_rotation)
{
    ...
    reverse_range(v, 0, 5);
    KOAN_EQ_INT(/*__*/ 6, v[0]);
    KOAN_EQ_INT(/*__*/ 1, v[5]);

    int w[6] = { 1, 2, 3, 4, 5, 6 };
    rotate_left(w, 6, 2);
    KOAN_EQ_INT(/*__*/ 3, w[0]);
    KOAN_EQ_INT(/*__*/ 4, w[1]);
    KOAN_EQ_INT(/*__*/ 2, w[5]);

    /* Rotating by the length is a no-op, and by more than the length wraps. */
    int x[3] = { 1, 2, 3 };
    rotate_left(x, 3, 3);
    KOAN_EQ_INT(/*__*/ 1, x[0]);

    rotate_left(x, 3, 4);          /* same as rotating by 1 */
    KOAN_EQ_INT(/*__*/ 2, x[0]);
}
