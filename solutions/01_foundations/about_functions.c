...
{
    KOAN_EQ_INT(/*__*/ 42, hidden_helper(21));
}
...
{
    KOAN_EQ_INT(/*__*/ 1, next_id());
    KOAN_EQ_INT(/*__*/ 2, next_id());
    KOAN_EQ_INT(/*__*/ 3, next_id());
}
...
KOAN_DELIBERATE_BEGIN
    {
        ...
        int value = 2;
        KOAN_EQ_INT(/*__*/ 2, value);
    }
KOAN_DELIBERATE_END
    KOAN_EQ_INT(/*__*/ 1, value);
}
...
{
    KOAN_EQ_INT(/*__*/ 1, (int)factorial(0));
    KOAN_EQ_INT(/*__*/ 120, (int)factorial(5));
}
...
{
    KOAN_EQ_SZ(/*__SZ*/ 4, count_char("mississippi", 's'));
    KOAN_EQ_SZ(/*__SZ*/ 4, count_char("mississippi", 'i'));
    KOAN_EQ_SZ(/*__SZ*/ 2, count_char("mississippi", 'p'));
    KOAN_EQ_SZ(/*__SZ*/ 0, count_char("mississippi", 'z'));
}
...
{
    KOAN_EQ_STR(/*__STR*/ "static storage outlives the call", safe_static());

    char buf[64];
    safe_caller_buffer(buf, sizeof buf);
    KOAN_EQ_STR(/*__STR*/ "the caller owns this", buf);
}
...
KOAN(assembling_a_tokeniser)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, split("alpha,beta,gamma", ',', parts, 4));
    KOAN_EQ_STR(/*__STR*/ "alpha", parts[0]);
    KOAN_EQ_STR(/*__STR*/ "gamma", parts[2]);

    /* An empty field between two separators is still a field. */
    KOAN_EQ_SZ(/*__SZ*/ 3, split("a,,c", ',', parts, 4));
    KOAN_EQ_STR(/*__STR*/ "", parts[1]);

    /* And the cap is respected rather than overrun. */
    KOAN_EQ_SZ(/*__SZ*/ 4, split("1,2,3,4,5,6", ',', parts, 4));
}
