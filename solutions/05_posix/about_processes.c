...
KOAN(fork_returns_twice)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, WIFEXITED(status));
    KOAN_EQ_INT(/*__*/ 7, WEXITSTATUS(status));
}
...
KOAN(a_signalled_child_is_reported_differently)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, WIFEXITED(status));
    KOAN_EQ_INT(/*__*/ 1, WIFSIGNALED(status));
    KOAN_EQ_INT(/*__*/ SIGKILL, WTERMSIG(status));
}
...
KOAN(a_pipe_carries_bytes_between_processes)
{
    ...
    KOAN_TRUE(got > 0);
    KOAN_EQ_STR(/*__STR*/ "from the child", buf);
    KOAN_EQ_SZ(/*__SZ*/ 14, (size_t)got);
}
...
KOAN(exec_replaces_the_program_and_dup2_redirects_it)
{
    ...
    KOAN_TRUE(got > 0);
    KOAN_EQ_STR(/*__STR*/ "replaced", buf);
    KOAN_EQ_INT(/*__*/ 0, WEXITSTATUS(status));
}
...
KOAN(signal_handlers_run_between_instructions)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, sigaction(SIGUSR1, &sa, &previous));

    signals_seen = 0;
    raise(SIGUSR1);
    raise(SIGUSR1);

    KOAN_EQ_INT(/*__*/ 2, signals_seen);
...
KOAN(assembling_a_command_runner)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, capture(echo_args, buf, sizeof buf, &ok));
    KOAN_EQ_INT(/*__*/ 1, ok);
    ...
    KOAN_EQ_INT(/*__*/ 127, capture(missing, buf, sizeof buf, &ok));
}
