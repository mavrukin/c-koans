...
KOAN(readdir_includes_dot_and_dotdot)
{
    ...
    KOAN_EQ_INT(/*__*/ 2, dots);
    KOAN_EQ_INT(/*__*/ 3, named);      /* a.txt, b.txt, sub */
    KOAN_EQ_INT(/*__*/ 5, entries);
...
KOAN(an_entry_is_a_name_not_a_path)
{
    ...
    char path[512];
    KOAN_EQ_INT(/*__*/ 1, join_path(path, sizeof path, TREE, "a.txt"));
    KOAN_EQ_STR(/*__STR*/ "koan_tree/a.txt", path);

    struct stat st;
    KOAN_EQ_INT(/*__*/ 0, stat(path, &st));
    KOAN_EQ_INT(/*__*/ 3, (int)st.st_size);

    /* Statting the bare name looks in the *current* directory instead, and
     * usually fails — or worse, succeeds on the wrong file. */
    errno = 0;
    KOAN_EQ_INT(/*__*/ -1, stat("a.txt", &st));

    /* A path too long for the buffer is refused rather than truncated. */
    char tiny[8];
    KOAN_EQ_INT(/*__*/ 0, join_path(tiny, sizeof tiny, TREE, "a.txt"));
...
KOAN(stat_distinguishes_files_from_directories)
{
    ...
    stat(path, &st);
    KOAN_EQ_INT(/*__*/ 1, S_ISDIR(st.st_mode) != 0);
    KOAN_EQ_INT(/*__*/ 0, S_ISREG(st.st_mode) != 0);

    join_path(path, sizeof path, TREE, "b.txt");
    stat(path, &st);
    KOAN_EQ_INT(/*__*/ 1, S_ISREG(st.st_mode) != 0);
    KOAN_EQ_INT(/*__*/ 2, (int)st.st_size);
...
KOAN(the_working_directory_is_process_wide)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, chdir(TREE));

    /* Relative paths now resolve inside the tree. */
    struct stat st;
    KOAN_EQ_INT(/*__*/ 0, stat("a.txt", &st));
    KOAN_EQ_INT(/*__*/ 3, (int)st.st_size);
...
KOAN(assembling_a_recursive_walk)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, s.files);
    KOAN_EQ_SZ(/*__SZ*/ 6, s.total_bytes);

    /* One directory below the root. */
    KOAN_EQ_SZ(/*__SZ*/ 1, s.dirs);

    /* The walk reached depth 1 and no further. */
    KOAN_EQ_SZ(/*__SZ*/ 1, s.max_depth_seen);
    KOAN_EQ_INT(/*__*/ 0, s.depth_exceeded);

    /* A depth limit of zero refuses to descend, and says so. */
    WalkStats shallow = { 0 };
    walk(TREE, 0, 0, &shallow);
    KOAN_EQ_SZ(/*__SZ*/ 2, shallow.files);        /* only the top level */
    KOAN_EQ_SZ(/*__SZ*/ 5, shallow.total_bytes);
    KOAN_EQ_INT(/*__*/ 1, shallow.depth_exceeded);

    /* Walking something that is not a directory yields nothing, not a crash. */
    WalkStats none = { 0 };
    walk("koan_tree/a.txt", 0, 16, &none);
    KOAN_EQ_SZ(/*__SZ*/ 0, none.files);
