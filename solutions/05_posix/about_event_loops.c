...
KOAN(a_non_blocking_read_returns_eagain)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, set_nonblocking(sv[0]));
    ...
    errno = 0;
    KOAN_EQ_INT(/*__*/ -1, (int)read(sv[0], buf, sizeof buf));
    KOAN_TRUE(errno == EAGAIN || errno == EWOULDBLOCK);

    /* Once data arrives, the same call succeeds. */
    write(sv[1], "hi", 2);
    KOAN_EQ_INT(/*__*/ 2, (int)read(sv[0], buf, sizeof buf));
...
KOAN(poll_reports_which_descriptors_are_ready)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, poll(fds, 2, 0));

    /* Write to the second pair only. */
    write(b[1], "x", 1);

    KOAN_EQ_INT(/*__*/ 1, poll(fds, 2, 100));

    /* The count says "one", but only revents says *which* one. */
    KOAN_EQ_INT(/*__*/ 0, fds[0].revents & POLLIN);
...
KOAN(the_timeout_distinguishes_three_outcomes)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, poll(&p, 1, 10));
    KOAN_EQ_INT(/*__*/ 0, p.revents);

    write(sv[1], "x", 1);
    KOAN_EQ_INT(/*__*/ 1, poll(&p, 1, 10));
...
KOAN(a_closed_peer_shows_as_readable)
{
    ...
    char buf[8];
    KOAN_EQ_INT(/*__*/ 0, (int)read(sv[0], buf, sizeof buf));   /* end of stream */
...
KOAN(assembling_a_single_threaded_event_loop)
{
    ...
    KOAN_EQ_INT(/*__*/ 4, l.count);        /* listener + three clients */
    ...
    char buf[16] = {};
    KOAN_EQ_INT(/*__*/ 2, (int)read(c1, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "aa", buf);

    memset(buf, 0, sizeof buf);
    KOAN_EQ_INT(/*__*/ 2, (int)read(c2, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "bb", buf);

    memset(buf, 0, sizeof buf);
    KOAN_EQ_INT(/*__*/ 2, (int)read(c3, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "cc", buf);

    /* A client hanging up is noticed and its slot reclaimed. */
    close(c2);
    for (int i = 0; i < 4; i++) loop_step(&l, 50);
    KOAN_EQ_INT(/*__*/ 1, l.disconnects);
    KOAN_EQ_INT(/*__*/ 3, l.count);
    ...
    for (int i = 0; i < 4; i++) loop_step(&l, 50);
    KOAN_EQ_INT(/*__*/ 2, (int)read(c3, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "zz", buf);

    close(c1); close(c3);
    for (int i = 0; i < 4; i++) loop_step(&l, 50);
    KOAN_EQ_INT(/*__*/ 3, l.disconnects);
    KOAN_EQ_INT(/*__*/ 1, l.count);        /* only the listener remains */
