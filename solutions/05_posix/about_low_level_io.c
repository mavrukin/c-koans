...
{
    KOAN_EQ_INT(/*__*/ 0, STDIN_FILENO);
    KOAN_EQ_INT(/*__*/ 1, STDOUT_FILENO);
    KOAN_EQ_INT(/*__*/ 2, STDERR_FILENO);
    ...
    errno = 0;
    char byte;
    KOAN_EQ_INT(/*__*/ -1, (int)read(fd, &byte, 1));
    KOAN_EQ_INT(/*__*/ EBADF, errno);
}
...
KOAN(open_flags_are_a_bitmask)
{
    ...
    KOAN_EQ_INT(/*__*/ -1, second);
    KOAN_EQ_INT(/*__*/ EEXIST, errno);

    /* Opening for reading something that does not exist gives ENOENT. */
    errno = 0;
    KOAN_EQ_INT(/*__*/ -1, open("koan_fd_absent.tmp", O_RDONLY));
    KOAN_EQ_INT(/*__*/ ENOENT, errno);
...
KOAN(short_transfers_require_a_loop)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, write_full(fd, payload, sizeof payload));
    lseek(fd, 0, SEEK_SET);

    char back[5000] = {};
    KOAN_EQ_INT(/*__*/ 5000, (int)read_full(fd, back, sizeof back));
    KOAN_EQ_INT(/*__*/ 0, memcmp(payload, back, sizeof payload));

    /* At end of file read_full returns what it managed, not an error. */
    KOAN_EQ_INT(/*__*/ 0, (int)read_full(fd, back, 10));
...
KOAN(lseek_returns_the_new_offset)
{
    ...
    KOAN_EQ_INT(/*__*/ 10, (int)lseek(fd, 0, SEEK_END));
    KOAN_EQ_INT(/*__*/ 0, (int)lseek(fd, 0, SEEK_SET));
    KOAN_EQ_INT(/*__*/ 3, (int)lseek(fd, 3, SEEK_CUR));

    char c;
    read(fd, &c, 1);
    KOAN_EQ_CHR(/*__CHR*/ '3', c);

    /* Seeking past the end is legal; it creates a hole when written to. */
    KOAN_EQ_INT(/*__*/ 20, (int)lseek(fd, 20, SEEK_SET));
    write_full(fd, "end", 3);
    KOAN_EQ_INT(/*__*/ 23, (int)lseek(fd, 0, SEEK_END));
...
KOAN(dup2_redirects_a_descriptor)
{
    ...
    char buf[16] = {};
    read_full(fd, buf, 8);
    KOAN_EQ_STR(/*__STR*/ "captured", buf);
...
KOAN(stat_reports_type_and_size)
{
    ...
    struct stat st;
    KOAN_EQ_INT(/*__*/ 0, stat(path, &st));

    KOAN_EQ_INT(/*__*/ 12, (int)st.st_size);
    KOAN_EQ_INT(/*__*/ 1, S_ISREG(st.st_mode) != 0);
    KOAN_EQ_INT(/*__*/ 0, S_ISDIR(st.st_mode) != 0);

    /* The permission bits are the low nine, and we created it 0600. */
    KOAN_EQ_INT(/*__*/ 0600, st.st_mode & 0777);

    /* A directory is a different type with the same call. */
    struct stat dir;
    KOAN_EQ_INT(/*__*/ 0, stat(".", &dir));
    KOAN_EQ_INT(/*__*/ 1, S_ISDIR(dir.st_mode) != 0);

    /* Statting what is not there fails with ENOENT. */
    errno = 0;
    KOAN_EQ_INT(/*__*/ -1, stat("koan_fd_absent.tmp", &st));
    KOAN_EQ_INT(/*__*/ ENOENT, errno);
...
KOAN(an_unlinked_file_lives_until_its_last_descriptor_closes)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, unlink(path));

    /* The name is gone... */
    struct stat st;
    KOAN_EQ_INT(/*__*/ -1, stat(path, &st));

    /* ...but the bytes are not. */
    char buf[16] = {};
    lseek(fd, 0, SEEK_SET);
    KOAN_EQ_INT(/*__*/ 10, (int)read_full(fd, buf, 10));
    KOAN_EQ_STR(/*__STR*/ "still here", buf);
...
KOAN(assembling_a_file_copier)
{
    ...
    KOAN_EQ_INT(/*__*/ COPY_OK, r);
    KOAN_EQ_SZ(/*__SZ*/ 10000, copied);

    /* Verify byte for byte, which is the only verification worth doing. */
    struct stat a, b;
    stat(src, &a);
    stat(dst, &b);
    KOAN_EQ_INT(/*__*/ 1, a.st_size == b.st_size);

    int  check = open(dst, O_RDONLY);
    char back[10000] = {};
    KOAN_EQ_INT(/*__*/ 10000, (int)read_full(check, back, sizeof back));
    KOAN_EQ_INT(/*__*/ 0, memcmp(payload, back, sizeof payload));
    close(check);

    /* A missing source is reported, and no destination is created. */
    unlink(dst);
    KOAN_EQ_INT(/*__*/ COPY_NO_SOURCE, copy_file("koan_absent.tmp", dst, &copied));
    KOAN_EQ_INT(/*__*/ -1, stat(dst, &b));

    /* Copying an empty file succeeds and copies nothing. */
    int empty = scratch_fd("koan_copy_empty.tmp", nullptr);
    close(empty);
    KOAN_EQ_INT(/*__*/ COPY_OK, copy_file("koan_copy_empty.tmp", dst, &copied));
    KOAN_EQ_SZ(/*__SZ*/ 0, copied);
