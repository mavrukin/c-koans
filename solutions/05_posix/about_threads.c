...
KOAN(a_thread_runs_a_function_and_returns_a_pointer)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, pthread_create(&t, nullptr, return_a_number, &input));

    void *result = nullptr;
    KOAN_EQ_INT(/*__*/ 0, pthread_join(t, &result));
    KOAN_EQ_INT(/*__*/ 42, (int)(intptr_t)result);
}
...
KOAN(a_mutex_makes_read_modify_write_indivisible)
{
    ...
    KOAN_EQ_INT(/*__*/ 40000, (int)s.counter);
}
...
KOAN(atomics_make_one_operation_indivisible)
{
    ...
    KOAN_EQ_INT(/*__*/ 40000, (int)atomic_load(&s.counter));
...
KOAN(thread_local_gives_each_thread_its_own_copy)
{
    ...
    KOAN_EQ_INT(/*__*/ 10, (int)(intptr_t)a_out);
    KOAN_EQ_INT(/*__*/ 20, (int)(intptr_t)b_out);

    /* And the main thread's copy was never touched by either. */
    KOAN_EQ_INT(/*__*/ 1, per_thread_value);
}
...
KOAN(c11_threads_are_pthreads_renamed)
{
    ...
    KOAN_EQ_INT(/*__*/ thrd_success,
                thrd_create(&t, standard_thread_body, &input));

    int result = 0;
    KOAN_EQ_INT(/*__*/ thrd_success, thrd_join(t, &result));
    KOAN_EQ_INT(/*__*/ 42, result);

    /* mtx_t and cnd_t mirror pthread_mutex_t and pthread_cond_t exactly. */
    mtx_t m;
    KOAN_EQ_INT(/*__*/ thrd_success, mtx_init(&m, mtx_plain));
    KOAN_EQ_INT(/*__*/ thrd_success, mtx_lock(&m));
    KOAN_EQ_INT(/*__*/ thrd_success, mtx_unlock(&m));
    mtx_destroy(&m);
}
...
KOAN(assembling_a_blocking_queue)
{
    ...
    KOAN_EQ_INT(/*__*/ 5050, (int)total);
    KOAN_EQ_INT(/*__*/ 100, taken);

    /* And pushing to a closed queue is refused rather than silently lost. */
    KOAN_EQ_INT(/*__*/ 0, queue_push(&q, 1));
