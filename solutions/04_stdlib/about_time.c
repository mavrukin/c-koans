...
KOAN(time_t_counts_seconds_from_an_epoch)
{
    ...
    time_t later = now + 60;
    KOAN_EQ_DBL(/*__DBL*/ 60.0, difftime(later, now), 1e-9);
    KOAN_EQ_DBL(/*__DBL*/ -60.0, difftime(now, later), 1e-9);
}
...
KOAN(struct_tm_fields_have_odd_bases)
{
    ...
    KOAN_EQ_INT(/*__*/ 101, utc.tm_year);
    KOAN_EQ_INT(/*__*/ 2001, utc.tm_year + 1900);

    /* tm_mon is 0-based: September is 8. */
    KOAN_EQ_INT(/*__*/ 8, utc.tm_mon);

    /* tm_mday is 1-based, unlike tm_mon. There is no reason for this. */
    KOAN_EQ_INT(/*__*/ 9, utc.tm_mday);

    KOAN_EQ_INT(/*__*/ 1, utc.tm_hour);
    KOAN_EQ_INT(/*__*/ 46, utc.tm_min);
    KOAN_EQ_INT(/*__*/ 40, utc.tm_sec);

    /* tm_wday is 0 for Sunday. 2001-09-09 was a Sunday. */
    KOAN_EQ_INT(/*__*/ 0, utc.tm_wday);

    /* tm_yday is 0-based day of the year. */
    KOAN_EQ_INT(/*__*/ 251, utc.tm_yday);
}
...
KOAN(strftime_formats_a_broken_down_time)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 10, strftime(buf, sizeof buf, "%Y-%m-%d", &utc));
    KOAN_EQ_STR(/*__STR*/ "2001-09-09", buf);

    strftime(buf, sizeof buf, "%H:%M:%S", &utc);
    KOAN_EQ_STR(/*__STR*/ "01:46:40", buf);

    /* ISO 8601, which is the only format worth writing to a log. */
    strftime(buf, sizeof buf, "%Y-%m-%dT%H:%M:%SZ", &utc);
    KOAN_EQ_STR(/*__STR*/ "2001-09-09T01:46:40Z", buf);

    /* Too small a buffer writes nothing and returns zero. */
    char tiny[4];
    KOAN_EQ_SZ(/*__SZ*/ 0, strftime(tiny, sizeof tiny, "%Y-%m-%d", &utc));
}
...
KOAN(mktime_normalises_out_of_range_fields)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, t.tm_mon);       /* February */
    KOAN_EQ_INT(/*__*/ 1, t.tm_mday);
    ...
    mktime(&plus90);
    KOAN_EQ_INT(/*__*/ 3, plus90.tm_mon);  /* April */
    KOAN_EQ_INT(/*__*/ 1, plus90.tm_mday);
}
...
KOAN(the_shared_buffer_forms_clobber_each_other)
{
    ...
    KOAN_EQ_INT(/*__*/ 101, year_before);      /* 2001 */
    ...
    KOAN_EQ_INT(/*__*/ 133, first->tm_year);   /* 2033, not 2001 */
    KOAN_EQ_INT(/*__*/ 1, first->tm_year == second->tm_year);
    ...
    KOAN_EQ_INT(/*__*/ 101, ta.tm_year);       /* 2001 */
    KOAN_EQ_INT(/*__*/ 133, tb.tm_year);       /* 2033 */
...
KOAN(clock_measures_processor_time)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, sink == 1999999L * 2000000L / 2);
...
KOAN(timespec_get_has_nanosecond_fields)
{
    ...
    KOAN_EQ_INT(/*__*/ TIME_UTC, timespec_get(&ts, TIME_UTC));
...
KOAN(assembling_a_duration_formatter)
{
    ...
    KOAN_EQ_STR(/*__STR*/ "0s", format_duration(0, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "45s", format_duration(45, buf, sizeof buf));

    /* 90 seconds carries into minutes. */
    KOAN_EQ_STR(/*__STR*/ "1m30s", format_duration(90, buf, sizeof buf));

    /* Exactly one hour: the minutes field is present and zero. */
    KOAN_EQ_STR(/*__STR*/ "1h0m", format_duration(3600, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "2h30m", format_duration(9000, buf, sizeof buf));

    /* A day and a half. */
    KOAN_EQ_STR(/*__STR*/ "1d12h", format_duration(129600, buf, sizeof buf));

    /* The boundary that catches people: 59s stays seconds, 60s becomes
     * a minute with a zero seconds field. */
    KOAN_EQ_STR(/*__STR*/ "59s", format_duration(59, buf, sizeof buf));
    KOAN_EQ_STR(/*__STR*/ "1m0s", format_duration(60, buf, sizeof buf));

    /* And a real elapsed measurement, using difftime rather than subtraction. */
    time_t start = 1000000000;
    time_t end   = start + 3661;
    KOAN_EQ_STR(/*__STR*/ "1h1m",
