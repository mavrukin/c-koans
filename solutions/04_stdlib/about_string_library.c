...
KOAN(strncpy_does_not_always_terminate)
{
    ...
    KOAN_EQ_STR(/*__STR*/ "abc", truncated);
    KOAN_EQ_SZ(/*__SZ*/ 3, strlen(truncated));
}
...
KOAN(snprintf_is_the_portable_bounded_copy)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, copy_bounded(buf, sizeof buf, "short"));
    KOAN_EQ_STR(/*__STR*/ "short", buf);

    KOAN_EQ_INT(/*__*/ 0, copy_bounded(buf, sizeof buf, "far too long"));
    KOAN_EQ_STR(/*__STR*/ "far too", buf);          /* terminated regardless */
    KOAN_EQ_SZ(/*__SZ*/ 7, strlen(buf));
}
...
KOAN(comparisons_report_order_not_distance)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, memcmp("abc", "abd", 2));   /* only the first two */
    ...
    KOAN_EQ_INT(/*__*/ 0, strncmp("prefix_rest", "prefix_other", 7));
...
KOAN(strspn_and_strcspn_measure_runs)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, strspn(line, " "));            /* leading spaces */
    KOAN_EQ_SZ(/*__SZ*/ 5, strspn(line, " 0123456789"));  /* spaces + digits */

    /* strcspn: how far to the first character in the set. */
    KOAN_EQ_SZ(/*__SZ*/ 3, strcspn(line, "0123456789"));
    KOAN_EQ_SZ(/*__SZ*/ 8, strcspn(line, "@"));           /* not found: whole length */

    /* strpbrk returns a pointer to the first match instead of an offset. */
    const char *digit = strpbrk(line, "0123456789");
    KOAN_EQ_CHR(/*__CHR*/ '4', *digit);
}
...
KOAN(strtok_mutates_and_remembers)
{
    ...
    KOAN_EQ_STR(/*__STR*/ "a", first);

    /* The separator was overwritten with a NUL, in place. */
    KOAN_EQ_CHR(/*__CHR*/ '\0', text[1]);

    KOAN_EQ_STR(/*__STR*/ "b", strtok(nullptr, ","));
    ...
    KOAN_EQ_STR(/*__STR*/ "c", strtok(nullptr, ","));
...
KOAN(ctype_needs_an_unsigned_char_cast)
{
    char word[] = "koan42";
    upcase(word);
    KOAN_EQ_STR(/*__STR*/ "KOAN42", word);

    KOAN_EQ_INT(/*__*/ 1, isdigit((unsigned char)'7') != 0);
    KOAN_EQ_INT(/*__*/ 0, isdigit((unsigned char)'x') != 0);
    KOAN_EQ_INT(/*__*/ 1, isalpha((unsigned char)'x') != 0);
    KOAN_EQ_INT(/*__*/ 1, isspace((unsigned char)'\t') != 0);

    /* toupper leaves anything that is not a lowercase letter alone. */
    KOAN_EQ_INT(/*__*/ '4', toupper((unsigned char)'4'));
}
...
KOAN(assembling_a_csv_splitter)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 3, split_csv("alpha,beta,gamma", ',', fields, 4));
    KOAN_EQ_INT(/*__*/ 1, field_equals(fields[0], "alpha"));
    KOAN_EQ_INT(/*__*/ 1, field_equals(fields[2], "gamma"));
    KOAN_EQ_SZ(/*__SZ*/ 4, fields[1].len);

    /* Empty fields are preserved, which is the whole point. */
    KOAN_EQ_SZ(/*__SZ*/ 4, split_csv("a,,b,", ',', fields, 4));
    KOAN_EQ_SZ(/*__SZ*/ 0, fields[1].len);
    KOAN_EQ_INT(/*__*/ 1, field_equals(fields[3], ""));

    /* A line with no separator is one field. */
    KOAN_EQ_SZ(/*__SZ*/ 1, split_csv("solo", ',', fields, 4));
    KOAN_EQ_INT(/*__*/ 1, field_equals(fields[0], "solo"));

    /* The empty line is one empty field, not zero fields. */
    KOAN_EQ_SZ(/*__SZ*/ 1, split_csv("", ',', fields, 4));
    KOAN_EQ_SZ(/*__SZ*/ 0, fields[0].len);

    /* Overflow is reported rather than hidden: the count exceeds the cap. */
    KOAN_EQ_SZ(/*__SZ*/ 6, split_csv("1,2,3,4,5,6", ',', fields, 4));

    /* And the input was never modified. */
    const char *original = "a,b";
    split_csv(original, ',', fields, 4);
    KOAN_EQ_STR(/*__STR*/ "a,b", original);
}
