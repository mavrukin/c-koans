...
KOAN(strlen_counts_bytes_not_characters)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 5, strlen(ascii));

    /* Each Greek letter is two bytes in UTF-8. */
    KOAN_EQ_SZ(/*__SZ*/ 6, strlen(greek));

    /* The emoji is four bytes, and is one character. */
    KOAN_EQ_SZ(/*__SZ*/ 4, strlen(emoji));
}
...
KOAN(utf8_lead_bytes_announce_the_length)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 1, utf8_sequence_length('A'));
    KOAN_EQ_SZ(/*__SZ*/ 2, utf8_sequence_length(greek[0]));
    KOAN_EQ_SZ(/*__SZ*/ 4, utf8_sequence_length(emoji[0]));

    /* Continuation bytes are not lead bytes, and never mistakable for one. */
    KOAN_EQ_INT(/*__*/ 1, is_continuation(greek[1]));
    KOAN_EQ_INT(/*__*/ 0, is_continuation(greek[0]));
    KOAN_EQ_SZ(/*__SZ*/ 0, utf8_sequence_length(greek[1]));

    /* ASCII bytes are never continuation bytes, which is why strchr(s, '/')
     * is still correct on UTF-8 text. */
    KOAN_EQ_INT(/*__*/ 0, is_continuation('/'));
}
...
{
    KOAN_EQ_SZ(/*__SZ*/ 5, utf8_count("hello"));
    KOAN_EQ_SZ(/*__SZ*/ 3, utf8_count("αβγ"));
    KOAN_EQ_SZ(/*__SZ*/ 1, utf8_count("\U0001F600"));

    /* Mixed text: 5 ASCII + 1 space + 3 Greek. */
    KOAN_EQ_SZ(/*__SZ*/ 9, utf8_count("hello αβγ"));

    /* And the byte count differs, which is the point. */
    KOAN_EQ_SZ(/*__SZ*/ 12, strlen("hello αβγ"));
}
...
KOAN(literals_can_declare_their_encoding)
{
    ...
    KOAN_EQ_INT(/*__*/ 0x61, (int)wide[0]);        /* 'a'   */
    KOAN_EQ_INT(/*__*/ 0x3B1, (int)wide[1]);       /* alpha */
    KOAN_EQ_INT(/*__*/ 0x1F600, (int)wide[2]);     /* emoji */
    KOAN_EQ_INT(/*__*/ 0, (int)wide[3]);           /* terminator */

    KOAN_EQ_SZ(/*__SZ*/ 4, sizeof(char32_t));

    /* The same text as UTF-8 needs a different number of elements. */
    KOAN_EQ_SZ(/*__SZ*/ 7, strlen("aα\U0001F600"));
}
...
KOAN(decoding_must_reject_malformed_input)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 1, utf8_decode("A", 1, &cp));
    KOAN_EQ_INT(/*__*/ 0x41, (int)cp);

    KOAN_EQ_SZ(/*__SZ*/ 2, utf8_decode("α", 2, &cp));
    KOAN_EQ_INT(/*__*/ 0x3B1, (int)cp);

    KOAN_EQ_SZ(/*__SZ*/ 4, utf8_decode("\U0001F600", 4, &cp));
    KOAN_EQ_INT(/*__*/ 0x1F600, (int)cp);

    /* A truncated sequence is rejected, not half-decoded. */
    KOAN_EQ_SZ(/*__SZ*/ 0, utf8_decode("α", 1, &cp));

    /* A continuation byte where a lead byte belongs. */
    KOAN_EQ_SZ(/*__SZ*/ 0, utf8_decode("\x80", 1, &cp));

    /* The security case: C0 AF is an over-long encoding of '/'. Accepting it
     * would let "..%C0%AF.." slip past a check for "../". */
    KOAN_EQ_SZ(/*__SZ*/ 0, utf8_decode("\xC0\xAF", 2, &cp));
}
...
KOAN(encoding_is_the_inverse)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 1, utf8_encode(0x41, buf));
    KOAN_EQ_STR(/*__STR*/ "A", buf);

    memset(buf, 0, sizeof buf);
    KOAN_EQ_SZ(/*__SZ*/ 2, utf8_encode(0x3B1, buf));
    KOAN_EQ_STR(/*__STR*/ "α", buf);

    memset(buf, 0, sizeof buf);
    KOAN_EQ_SZ(/*__SZ*/ 4, utf8_encode(0x1F600, buf));
    KOAN_EQ_STR(/*__STR*/ "\U0001F600", buf);

    /* A round trip must be exact — the real test of a codec. */
    char32_t back = 0;
    utf8_decode(buf, 4, &back);
    KOAN_EQ_INT(/*__*/ 0x1F600, (int)back);
}
...
KOAN(assembling_a_utf8_safe_truncation)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 5, utf8_truncate("hello world", 5, out, sizeof out));
    KOAN_EQ_STR(/*__STR*/ "hello", out);

    /* "αβγ" is 6 bytes. A budget of 5 must stop after two letters, not
     * three-and-a-half — so it writes 4 bytes, not 5. */
    KOAN_EQ_SZ(/*__SZ*/ 4, utf8_truncate("αβγ", 5, out, sizeof out));
    KOAN_EQ_SZ(/*__SZ*/ 2, utf8_count(out));

    /* The result is always valid UTF-8, which is the whole requirement. */
    char32_t cp = 0;
    KOAN_EQ_SZ(/*__SZ*/ 2, utf8_decode(out, strlen(out), &cp));
    KOAN_EQ_INT(/*__*/ 0x3B1, (int)cp);

    /* A budget smaller than the first character yields the empty string
     * rather than a broken byte. */
    KOAN_EQ_SZ(/*__SZ*/ 0, utf8_truncate("\U0001F600", 3, out, sizeof out));
    KOAN_EQ_STR(/*__STR*/ "", out);

    /* A budget larger than the text copies all of it. */
    KOAN_EQ_SZ(/*__SZ*/ 6, utf8_truncate("αβγ", 100, out, sizeof out));
    KOAN_EQ_SZ(/*__SZ*/ 3, utf8_count(out));
}
