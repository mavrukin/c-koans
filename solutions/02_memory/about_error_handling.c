...
KOAN(strtol_distinguishes_three_failures)
{
    ...
    KOAN_EQ_INT(/*__*/ PARSE_OK, parse_long("42", &v));
    KOAN_EQ_INT(/*__*/ 42, (int)v);

    KOAN_EQ_INT(/*__*/ PARSE_OK, parse_long("-7", &v));
    KOAN_EQ_INT(/*__*/ -7, (int)v);

    KOAN_EQ_INT(/*__*/ PARSE_EMPTY, parse_long("", &v));
    KOAN_EQ_INT(/*__*/ PARSE_EMPTY, parse_long("abc", &v));
    KOAN_EQ_INT(/*__*/ PARSE_JUNK, parse_long("42abc", &v));
    KOAN_EQ_INT(/*__*/ PARSE_JUNK, parse_long("42 ", &v));
    KOAN_EQ_INT(/*__*/ PARSE_RANGE,
    ...
    KOAN_EQ_INT(/*__*/ 0, atoi("abc"));
    KOAN_EQ_INT(/*__*/ 0, atoi("0"));
}
...
{
    KOAN_EQ_STR(/*__STR*/ "trailing characters", PARSE_MESSAGES[PARSE_JUNK]);
    KOAN_EQ_STR(/*__STR*/ "out of range", PARSE_MESSAGES[PARSE_RANGE]);
}
...
KOAN(assembling_a_failing_parser)
{
    ...
    KOAN_EQ_STR(/*__STR*/ "timeout", s.key);
    KOAN_EQ_INT(/*__*/ 30, (int)s.value);

    /* No separator at all: the offset points past the end of the input. */
    KOAN_EQ_INT(/*__*/ 0, parse_setting("timeout", &s));
    KOAN_EQ_INT(/*__*/ PARSE_JUNK, s.code);
    KOAN_EQ_SZ(/*__SZ*/ 7, s.offset);

    /* Empty key. */
    KOAN_EQ_INT(/*__*/ 0, parse_setting("=30", &s));
    KOAN_EQ_INT(/*__*/ PARSE_EMPTY, s.code);

    /* Bad value: the offset points at the value, not the start of the line. */
    KOAN_EQ_INT(/*__*/ 0, parse_setting("timeout=soon", &s));
    KOAN_EQ_INT(/*__*/ PARSE_EMPTY, s.code);
    KOAN_EQ_SZ(/*__SZ*/ 8, s.offset);

    /* Value out of range is a different failure from value being junk. */
    KOAN_EQ_INT(/*__*/ 0, parse_setting("timeout=99999999999999999999", &s));
    KOAN_EQ_INT(/*__*/ PARSE_RANGE, s.code);

    /* An over-long key is refused rather than truncated into the buffer. */
    char  long_line[128];
    memset(long_line, 'k', 64);
    memcpy(long_line + 64, "=1", 3);
    KOAN_EQ_INT(/*__*/ 0, parse_setting(long_line, &s));
    KOAN_EQ_INT(/*__*/ PARSE_RANGE, s.code);
}
