...
KOAN(fseek_moves_the_position)
{
    ...
    fseek(f, 0, SEEK_END);
    KOAN_EQ_INT(/*__*/ 10, (int)ftell(f));      /* the size */

    fseek(f, 3, SEEK_SET);
    KOAN_EQ_INT(/*__*/ 'd', fgetc(f));          /* index 3 */

    fseek(f, 2, SEEK_CUR);                      /* skip 'e','f' */
    KOAN_EQ_INT(/*__*/ 'g', fgetc(f));

    fseek(f, -2, SEEK_END);
    KOAN_EQ_INT(/*__*/ 'i', fgetc(f));
...
KOAN(fread_returns_elements_not_bytes)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 4, fwrite(out, sizeof *out, 4, f));

    rewind(f);

    int in[4] = {};
    KOAN_EQ_SZ(/*__SZ*/ 4, fread(in, sizeof *in, 4, f));
    KOAN_EQ_INT(/*__*/ 30, in[2]);

    /* Asking for more than remains gives a short count, not an error. */
    rewind(f);
    int big[10] = {};
    KOAN_EQ_SZ(/*__SZ*/ 4, fread(big, sizeof *big, 10, f));
    KOAN_EQ_INT(/*__*/ 1, feof(f));
...
KOAN(feof_is_true_only_after_a_failed_read)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, feof(f));         /* not yet, though nothing is left
                                             * after two more reads */
    fgetc(f);
    fgetc(f);
    KOAN_EQ_INT(/*__*/ 0, feof(f));         /* still not: we read exactly two */

    KOAN_EQ_INT(/*__*/ EOF, fgetc(f));      /* this read fails ... */
    KOAN_EQ_INT(/*__*/ 1, feof(f));         /* ... and only now is feof true */
...
KOAN(fgetc_returns_int_for_a_reason)
{
    ...
    KOAN_EQ_INT(/*__*/ 255, as_int);
    KOAN_EQ_INT(/*__*/ -1, as_char);
...
KOAN(sscanf_returns_the_assignment_count)
{
    ...
    KOAN_EQ_INT(/*__*/ 2, sscanf("12 34", "%d %d", &a, &b));
    KOAN_EQ_INT(/*__*/ 34, b);

    /* It stops at the first thing that does not match, and reports how far
     * it got. b is left untouched. */
    a = 0; b = 99;
    KOAN_EQ_INT(/*__*/ 1, sscanf("12 xy", "%d %d", &a, &b));
    KOAN_EQ_INT(/*__*/ 12, a);
    KOAN_EQ_INT(/*__*/ 99, b);

    /* %s is unbounded and will overflow. Always give it a width. */
    KOAN_EQ_INT(/*__*/ 1, sscanf("supercalifragilistic", "%15s", word));
    KOAN_EQ_SZ(/*__SZ*/ 15, strlen(word));

    /* Nothing matched at all. */
    KOAN_EQ_INT(/*__*/ 0, sscanf("xy", "%d", &a));
}
...
KOAN(assembling_a_record_file)
{
    ...
    Record read_back[4] = {};
    KOAN_EQ_SZ(/*__SZ*/ 2, records_read(f, read_back, 4));
    KOAN_EQ_STR(/*__STR*/ "ada", read_back[0].id);
    KOAN_EQ_INT(/*__*/ 95, read_back[1].score);
    fclose(f);
