...
KOAN(rows_are_contiguous)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 48, sizeof grid);
    KOAN_EQ_SZ(/*__SZ*/ 16, sizeof grid[0]);      /* one row */
    KOAN_EQ_SZ(/*__SZ*/ 4, sizeof grid[0][0]);

    /* Because it is one block, it can be read as a flat array. */
    const int *flat = &grid[0][0];
    KOAN_EQ_INT(/*__*/ 11, flat[5]);              /* row 1, column 1 */
    KOAN_EQ_INT(/*__*/ 22, flat[2 * 4 + 2]);

    /* The index arithmetic the compiler is doing for you. */
    KOAN_EQ_INT(/*__*/ 13, flat[1 * 4 + 3]);
}
...
KOAN(a_2d_array_decays_to_a_pointer_to_a_row)
{
    ...
    KOAN_EQ_INT(/*__*/ 36, sum_fixed(grid, 2));

    /* Stepping a pointer-to-row advances by a whole row. */
    int (*p)[4] = grid;
    KOAN_EQ_SZ(/*__SZ*/ 16, (size_t)((char *)(p + 1) - (char *)p));
    KOAN_EQ_INT(/*__*/ 5, p[1][0]);
}
...
KOAN(flat_buffers_take_any_shape)
{
    ...
    KOAN_EQ_INT(/*__*/ 4, at(data, 3, 1, 0));
    KOAN_EQ_INT(/*__*/ 6, at(data, 3, 1, 2));
    KOAN_EQ_INT(/*__*/ 5, at(data, 2, 2, 0));
    KOAN_EQ_INT(/*__*/ 3, at(data, 2, 1, 0));
}
...
{
    KOAN_EQ_INT(/*__*/ 10, sum_vla(5));
    KOAN_EQ_INT(/*__*/ 45, sum_vla(10));

    /* sizeof on a VLA is evaluated at runtime, unlike every other sizeof. */
KOAN_DELIBERATE_BEGIN
    size_t n = 7;
    int    v[n];
    KOAN_EQ_SZ(/*__SZ*/ 28, sizeof v);
    (void)v;
KOAN_DELIBERATE_END
}
...
KOAN(variably_modified_types_take_any_shape)
{
    ...
    KOAN_EQ_INT(/*__*/ 15, trace(3, a));
    KOAN_EQ_INT(/*__*/ 30, trace(2, b));
}
...
KOAN(flexible_array_members_pack_header_and_payload)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ sizeof(size_t), sizeof(String));

    String *s = string_new("koan");
    KOAN_EQ_SZ(/*__SZ*/ 4, s->len);
    KOAN_EQ_STR(/*__STR*/ "koan", s->text);

    /* The payload sits immediately after the header, in the same block. */
    KOAN_EQ_SZ(/*__SZ*/ sizeof(size_t),
...
KOAN(assembling_a_matrix)
{
    ...
    KOAN_EQ_INT(/*__*/ 19, mat_get(&product, 0, 0));
    KOAN_EQ_INT(/*__*/ 22, mat_get(&product, 0, 1));
    KOAN_EQ_INT(/*__*/ 43, mat_get(&product, 1, 0));
    KOAN_EQ_INT(/*__*/ 50, mat_get(&product, 1, 1));
    ...
    KOAN_EQ_INT(/*__*/ 0, mat_mul(&product, &a, &wide));
