...
{
    KOAN_EQ_INT(/*__*/ 1, close_enough(0.1 + 0.2, 0.3, 1e-12));
    KOAN_EQ_INT(/*__*/ 0, close_enough(1.0, 1.5, 1e-12));
     ...
     * beside 1e15. */
    KOAN_EQ_INT(/*__*/ 1, close_enough(1e15, 1e15 + 0.125, 1e-12));
}
...
KOAN(epsilon_is_the_gap_at_one)
{
    ...
    KOAN_EQ_INT(/*__*/ 53, DBL_MANT_DIG);           /* bits of significand */
    KOAN_EQ_INT(/*__*/ 24, FLT_MANT_DIG);
...
{
    KOAN_EQ_INT(/*__*/ 1, (double)9007199254740992LL == 9007199254740992.0);

    /* One past it collides with its neighbour. */
    int64_t n = 9007199254740993LL;                  /* 2^53 + 1 */
    KOAN_TRUE((int64_t)(double)n != n);
    KOAN_EQ_INT(/*__*/ 1, (double)n == 9007199254740992.0);
}
...
KOAN(there_are_two_zeros)
{
    ...
    KOAN_TRUE(pos == neg);
    KOAN_EQ_INT(/*__*/ 0, signbit(pos));
    KOAN_EQ_INT(/*__*/ 1, signbit(neg) != 0);
...
{
    KOAN_EQ_DBL(/*__DBL*/ 2.0, ceil(1.1), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ 1.0, floor(1.9), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ 1.0, trunc(1.9), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ -1.0, trunc(-1.9), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ -2.0, floor(-1.9), 1e-12);

    /* round() goes away from zero on a tie. */
    KOAN_EQ_DBL(/*__DBL*/ 1.0, round(0.5), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ 2.0, round(1.5), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ -1.0, round(-0.5), 1e-12);

    /* rint() honours the current mode, which by default is ties-to-even. */
    KOAN_EQ_DBL(/*__DBL*/ 0.0, rint(0.5), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ 2.0, rint(1.5), 1e-12);
    KOAN_EQ_DBL(/*__DBL*/ 2.0, rint(2.5), 1e-12);
}
...
KOAN(assembling_kahan_summation)
{
    ...
    KOAN_EQ_DBL(/*__DBL*/ 1000.0, kahan, 1e-12);
