...
{
    KOAN_EQ_INT(/*__*/ 0, file_scope_int);
    KOAN_EQ_SZ(/*__SZ*/ 0, strlen(file_scope_buf));
...
{
    KOAN_EQ_INT(/*__*/ 9, SQUARES[3]);
    KOAN_EQ_SZ(/*__SZ*/ 4, sizeof SQUARES / sizeof *SQUARES);
    ...
    KOAN_EQ_INT(/*__*/ 5, computed[0]);
    KOAN_EQ_INT(/*__*/ 0, computed[3]);
}
...
KOAN(compound_literal_lifetime_is_its_block)
{
...

    {
        ...
        KOAN_EQ_INT(/*__*/ 10, manhattan(inner));
        outer = inner;
    }
    ...
    KOAN_EQ_INT(/*__*/ 0, manhattan(safe_static_literal()));
}
...
KOAN(compound_literals_make_clean_arguments)
{
    ...
    KOAN_EQ_INT(/*__*/ 3, total_retries((Options){ .name = "fetch", .retries = 3 }));

    /* Unmentioned members are zeroed, so defaults are free. */
    Options o = (Options){ .name = "x" };
    KOAN_EQ_INT(/*__*/ 0, o.retries);
    KOAN_EQ_INT(/*__*/ 0, o.verbose);
...
KOAN_DELIBERATE_END
    KOAN_EQ_INT(/*__*/ 99, v[0]);
    KOAN_EQ_INT(/*__*/ 2, v[1]);

    /* Positional entries resume from the last designated index. */
    int w[6] = { [2] = 30, 40, 50 };
    KOAN_EQ_INT(/*__*/ 0, w[1]);
    KOAN_EQ_INT(/*__*/ 30, w[2]);
    KOAN_EQ_INT(/*__*/ 40, w[3]);
    KOAN_EQ_INT(/*__*/ 50, w[4]);
    KOAN_EQ_INT(/*__*/ 0, w[5]);
}
...
KOAN(designators_nest)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, r.topleft.x);
    KOAN_EQ_INT(/*__*/ 20, r.size.y);

    /* Anything not mentioned is still zeroed, at any depth. */
    Rect partial = { .size.x = 5 };
    KOAN_EQ_INT(/*__*/ 0, partial.topleft.x);
    KOAN_EQ_INT(/*__*/ 0, partial.size.y);
}
...
{
    KOAN_EQ_STR(/*__STR*/ "safe", PROFILES[MODE_SAFE].name);
    KOAN_EQ_INT(/*__*/ 5, PROFILES[MODE_SAFE].retries);

    /* Unmentioned members are zeroed even inside a designated table. */
    KOAN_EQ_INT(/*__*/ 0, PROFILES[MODE_FAST].verbose);
    KOAN_EQ_INT(/*__*/ 1, PROFILES[MODE_DEBUG].verbose);
    KOAN_EQ_INT(/*__*/ 0, PROFILES[MODE_DEBUG].timeout_ms);

    /* The overlay copies rather than aliases: the table is untouched. */
    Profile tuned = with_timeout(MODE_SAFE, 250);
    KOAN_EQ_INT(/*__*/ 250, tuned.timeout_ms);
    KOAN_EQ_INT(/*__*/ 5000, PROFILES[MODE_SAFE].timeout_ms);
    KOAN_EQ_STR(/*__STR*/ "safe", tuned.name);

    /* A compound literal supplies a one-off profile with no table entry. */
    Profile ad_hoc = (Profile){ .name = "once", .retries = 2 };
    KOAN_EQ_INT(/*__*/ 2, ad_hoc.retries);
    KOAN_EQ_INT(/*__*/ 0, ad_hoc.timeout_ms);
}
