...
KOAN(bool_is_a_keyword)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, yes);
    KOAN_EQ_INT(/*__*/ 0, no);
    KOAN_EQ_SZ(/*__SZ*/ 1, sizeof(bool));

    /* Converting to bool normalises to exactly 0 or 1, unlike a cast to int. */
    KOAN_EQ_INT(/*__*/ 1, (bool)42);
    KOAN_EQ_INT(/*__*/ 42, (int)42);
}
...
KOAN(constexpr_is_a_typed_compile_time_constant)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ 64, sizeof buf);

    /* And it has a type, unlike a #define. */
    KOAN_EQ_SZ(/*__SZ*/ sizeof(int), sizeof BUFFER_SIZE);
    KOAN_EQ_SZ(/*__SZ*/ sizeof(double), sizeof GOLDEN);

    /* A plain `const int` is NOT a constant expression in C, which is the
     * distinction that makes constexpr worth having. */
    KOAN_EQ_INT(/*__*/ 64, BUFFER_SIZE);
}
...
KOAN(auto_infers_the_type)
{
    ...
    KOAN_EQ_SZ(/*__SZ*/ sizeof(int), sizeof n);
    KOAN_EQ_SZ(/*__SZ*/ sizeof(double), sizeof d);
    KOAN_EQ_INT(/*__*/ 42, n);
    KOAN_EQ_STR(/*__STR*/ "text", s);
}
...
KOAN(attributes_are_standard_now)
{
    ...
    KOAN_EQ_INT(/*__*/ 42, doubled);

    /* Calling a deprecated function warns but still works. */
KOAN_DELIBERATE_BEGIN
    KOAN_EQ_INT(/*__*/ 42, old_way(21));
KOAN_DELIBERATE_END

    KOAN_EQ_INT(/*__*/ 1, unused_parameter_ok(0));
...
KOAN(checked_arithmetic_reports_overflow)
{
    ...
    KOAN_EQ_INT(/*__*/ 0, ckd_add(&result, 20, 22));
    KOAN_EQ_INT(/*__*/ 42, result);

    /* Returns true when it overflowed — and result holds the wrapped value,
     * which you must then not use. */
    KOAN_EQ_INT(/*__*/ 1, ckd_add(&result, INT_MAX, 1));

    KOAN_EQ_INT(/*__*/ 0, ckd_mul(&result, 1000, 1000));
    KOAN_EQ_INT(/*__*/ 1000000, result);
    KOAN_EQ_INT(/*__*/ 1, ckd_mul(&result, INT_MAX, 2));

    KOAN_EQ_INT(/*__*/ 1, ckd_sub(&result, INT_MIN, 1));
#else
...
#embed "assets/greeting.txt"
    ...
    KOAN_EQ_STR(/*__STR*/ "hello from a file", (const char *)greeting);
...
KOAN(assembling_a_fixed_point_type)
{
    ...
    KOAN_EQ_INT(/*__*/ 1, fixed_from_parts(&price, 19, 99));
    KOAN_EQ_STR(/*__STR*/ "19.99", fixed_text(price, buf, sizeof buf));

    KOAN_EQ_INT(/*__*/ 1, fixed_from_parts(&tax, 1, 60));
    KOAN_EQ_INT(/*__*/ 1, fixed_add(&total, price, tax));
    KOAN_EQ_STR(/*__STR*/ "21.59", fixed_text(total, buf, sizeof buf));

    /* The invariant is enforced rather than assumed. */
    KOAN_EQ_INT(/*__*/ 0, fixed_from_parts(&price, 5, 100));
    KOAN_EQ_INT(/*__*/ 0, fixed_from_parts(&price, 5, -1));
}
