...
{
    KOAN_EQ_STR(__STR, KOAN_BUILD_LABEL);
    ...
    KOAN_EQ_STR(__STR, joined);
    KOAN_EQ_SZ(__SZ, strlen(joined));
}
...
KOAN(the_preprocessor_knows_where_it_is)
{
    ...
    KOAN_EQ_INT(__, __LINE__ - line_here);
...
KOAN(a_header_is_pasted_in_not_imported)
{
    ...
    KOAN_EQ_INT(__, BUILD_DEMO_CONSTEXPR_ANSWER);
    KOAN_EQ_SZ(__SZ, sizeof BUILD_DEMO_CONSTEXPR_ANSWER);
...
KOAN(assembling_a_mental_model_of_the_build)
{
    ...
    KOAN_EQ_INT(__, s.definitions_of_add);

    /* And the link happens once, over all the objects at the end. */
    KOAN_EQ_INT(__, s.link_steps);
