...
KOAN(scanf_s_must_always_have_a_width)
{
    ...
    KOAN_EQ_INT(__, sscanf("abcdefghijkl", "%7s", small));
    KOAN_EQ_STR(__STR, small);
    KOAN_EQ_SZ(__SZ, strlen(small));
...
KOAN(whitespace_handling_is_not_uniform)
{
    ...
    KOAN_EQ_INT(__, sscanf("  1     2  ", "%d %d", &a, &b));
    KOAN_EQ_INT(__, b);

    /* %d skipped the leading spaces without being asked. */
    a = 0;
    KOAN_EQ_INT(__, sscanf("   7", "%d", &a));
    KOAN_EQ_INT(__, a);
...
KOAN(scansets_match_character_classes)
{
    ...
    KOAN_EQ_INT(__, sscanf("hello, world", "%31[^,]", field));
    KOAN_EQ_STR(__STR, field);

    /* Only digits. */
    char digits[16] = {};
    sscanf("2026abc", "%15[0-9]", digits);
    KOAN_EQ_STR(__STR, digits);
    ...
    KOAN_EQ_INT(__, sscanf("xyz", "%7[0-9]", untouched));
    KOAN_EQ_STR(__STR, untouched);
}
...
KOAN(fgets_keeps_the_newline)
{
    ...
    KOAN_EQ_STR(__STR, line);
    KOAN_EQ_SZ(__SZ, strlen(line));
...
KOAN(a_failed_scanf_does_not_consume_the_input)
{
    ...
    KOAN_EQ_INT(__, fscanf(f, "%d", &value));
    KOAN_EQ_INT(__, fgetc(f));       /* still at the 'o' */
    ungetc('o', f);
...
KOAN(fgets_then_parse_is_the_robust_pattern)
{
    ...
    KOAN_EQ_INT(__, read_int_line(f, &v));
...
KOAN(assembling_a_configuration_reader)
{
    ...
    KOAN_EQ_SZ(__SZ, r.accepted);
    KOAN_EQ_SZ(__SZ, r.rejected);

    /* "broken line" is on line 5 — comments and blanks still count as lines. */
    KOAN_EQ_INT(__, r.first_bad_line);

    /* Whitespace around both key and value was trimmed. */
    KOAN_EQ_STR(__STR, settings[0].key);
    KOAN_EQ_INT(__, (int)settings[0].value);
    KOAN_EQ_STR(__STR, settings[1].key);
    KOAN_EQ_INT(__, (int)settings[2].value);
    ...
    fclose(quiet);
    KOAN_EQ_SZ(__SZ, empty.accepted);
    KOAN_EQ_SZ(__SZ, empty.rejected);
    KOAN_EQ_INT(__, empty.first_bad_line);
}
