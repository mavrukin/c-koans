...
KOAN(strings_carry_a_terminator)
{
    ...
    KOAN_EQ_SZ(__SZ, sizeof word);   /* four letters plus the NUL */
    KOAN_EQ_SZ(__SZ, strlen(word));
    KOAN_EQ_CHR(__CHR, word[4]);
}
...
KOAN(literals_are_read_only)
{
    ...
    KOAN_EQ_CHR(__CHR, literal[0]);

    copy[0] = 'M';                        /* legal: copy is our own array */
    KOAN_EQ_STR(__STR, copy);
}
...
KOAN(strcmp_orders_by_first_difference)
{
    ...
    KOAN_EQ_INT(__, strncmp("abcdef", "abcXXX", 3));
}
...
KOAN(searching_returns_pointers_not_indices)
{
    ...
    KOAN_EQ_SZ(__SZ, (size_t)(space - s));

    const char *brown = strstr(s, "brown");
    KOAN_EQ_SZ(__SZ, (size_t)(brown - s));
    ...
    KOAN_EQ_STR(__STR, strrchr(path, '/') + 1);
    KOAN_EQ_STR(__STR, strrchr(path, '.'));
}
...
KOAN(assembling_a_bounded_string_builder)
{
    ...
    KOAN_EQ_STR(__STR, b.buf);
    KOAN_EQ_SZ(__SZ, b.len);
    KOAN_EQ_INT(__, b.overflowed);
    ...
    KOAN_EQ_STR(__STR, ok.buf);
    KOAN_EQ_INT(__, ok.overflowed);
}
