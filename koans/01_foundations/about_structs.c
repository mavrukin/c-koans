...
KOAN(padding_makes_order_matter)
{
    ...
    KOAN_EQ_SZ(__SZ, sizeof(Loose));
    KOAN_EQ_SZ(__SZ, sizeof(Tight));

    /* The members themselves account for only six bytes. */
    KOAN_EQ_SZ(__SZ, sizeof(char) + sizeof(int) + sizeof(char));
}
...
{
    KOAN_EQ_SZ(__SZ, offsetof(Loose, a));
    KOAN_EQ_SZ(__SZ, offsetof(Loose, b));
    KOAN_EQ_SZ(__SZ, offsetof(Loose, c));
...
KOAN(tagged_unions_remember_which_member_is_live)
{
    ...
    KOAN_EQ_STR(__STR, describe(i, buf, sizeof buf));
    KOAN_EQ_STR(__STR, describe(d, buf, sizeof buf));
    KOAN_EQ_STR(__STR, describe(t, buf, sizeof buf));

    /* An anonymous union's members are named as though they were the
     * struct's own, which is what makes this pattern readable. */
    KOAN_EQ_INT(__, i.as_int);
}
...
{
    KOAN_EQ_INT(__, STATUS_OK);
    KOAN_EQ_INT(__, STATUS_WARN);
    KOAN_EQ_INT(__, STATUS_ERROR);
    KOAN_EQ_INT(__, STATUS_FATAL);
}
...
KOAN(bitfields_pack_small_values)
{
    ...
    KOAN_EQ_INT(__, h.version);
    KOAN_EQ_INT(__, h.flags);
    KOAN_EQ_INT(__, h.length);

    /* Sixteen bits of payload fit inside a single unsigned. */
    KOAN_EQ_SZ(__SZ, sizeof(Header));
...
KOAN(assembling_a_wire_format)
{
    ...
    KOAN_EQ_INT(__, back.version);
    KOAN_EQ_INT(__, back.flags);
    KOAN_EQ_INT(__, back.length);

    /* A round trip must be lossless — the real test of a serialiser. */
    KOAN_EQ_INT(__, pack_header(unpack_header(wire)) == wire);
}
