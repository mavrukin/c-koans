...
KOAN(the_three_standard_streams_are_ordinary_files)
{
    ...
    KOAN_EQ_INT(__, fprintf(stdout, "%s", ""));
...
KOAN(buffering_decides_when_bytes_leave)
{
    ...
    FILE *w = fopen(path, "w");
    KOAN_TRUE(w != nullptr);
    KOAN_EQ_INT(__, setvbuf(w, nullptr, _IOFBF, 4096));
    ...
    char  seen[32] = {};
    KOAN_EQ_SZ(__SZ, fread(seen, 1, sizeof seen - 1, peek));
    fclose(peek);
...
KOAN(a_failed_write_sets_the_error_flag)
{
    ...
    KOAN_EQ_INT(__, ferror(r) != 0);

    clearerr(r);
    KOAN_EQ_INT(__, ferror(r));
...
KOAN(ungetc_gives_one_character_of_lookahead)
{
    ...
    ungetc(c, f);
    KOAN_EQ_INT(__, fgetc(f));      /* the same character again */
...
KOAN(remove_and_rename_are_standard_c)
{
    ...
    KOAN_EQ_INT(__, rename(first, second));
...
KOAN(assembling_a_growing_line_reader)
{
    ...
    KOAN_EQ_INT(__, read_line(f, &buf, &cap, &len));
    KOAN_EQ_STR(__STR, buf);
    KOAN_EQ_SZ(__SZ, len);
    ...
    KOAN_EQ_SZ(__SZ, len);
    KOAN_TRUE(cap > 300);
    KOAN_EQ_CHR(__CHR, buf[299]);
    KOAN_EQ_CHR(__CHR, buf[300]);
