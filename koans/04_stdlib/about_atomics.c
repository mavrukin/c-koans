...
KOAN(atomic_load_and_store_are_indivisible)
{
    ...
    KOAN_EQ_INT(__, atomic_load(&counter));
...
KOAN(fetch_operations_return_the_previous_value)
{
    ...
    KOAN_EQ_INT(__, atomic_fetch_add(&n, 5));
    KOAN_EQ_INT(__, atomic_load(&n));

    KOAN_EQ_INT(__, atomic_fetch_sub(&n, 5));
    KOAN_EQ_INT(__, atomic_load(&n));

    atomic_int flags;
    atomic_init(&flags, 0b1010);
    KOAN_EQ_INT(__, atomic_fetch_or(&flags, 0b0101));
    KOAN_EQ_INT(__, atomic_load(&flags));

    KOAN_EQ_INT(__, atomic_fetch_and(&flags, 0b0011));
    KOAN_EQ_INT(__, atomic_load(&flags));

    /* atomic_exchange sets a new value and hands back the old one. */
    KOAN_EQ_INT(__, atomic_exchange(&flags, 0));
    KOAN_EQ_INT(__, atomic_load(&flags));
}
...
KOAN(some_atomics_are_implemented_with_locks)
{
    atomic_int small;
    atomic_init(&small, 0);
    KOAN_EQ_INT(__, atomic_is_lock_free(&small));
    ...
    KOAN_EQ_INT(__, atomic_flag_test_and_set(&gate));
    KOAN_EQ_INT(__, atomic_flag_test_and_set(&gate));
...
KOAN(relaxed_is_atomic_but_unordered)
{
     ...
     * atomicity. */
    KOAN_EQ_INT(__, (int)atomic_load(&c.hits));
}
...
KOAN(fences_order_without_an_object)
{
    ...
    KOAN_EQ_INT(__, atomic_load_explicit(&a, memory_order_relaxed));
    KOAN_EQ_INT(__, atomic_load_explicit(&b, memory_order_relaxed));
}
...
KOAN(assembling_a_lock_free_stack)
{
    ...
    KOAN_EQ_INT(__, popped);
    KOAN_EQ_INT(__, total);
