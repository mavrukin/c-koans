...
KOAN(generic_selects_on_type)
{
    ...
    KOAN_EQ_STR(__STR, TYPE_NAME(i));
    KOAN_EQ_STR(__STR, TYPE_NAME(d));
    KOAN_EQ_STR(__STR, TYPE_NAME(s));
    KOAN_EQ_STR(__STR, TYPE_NAME(cs));

    /* No match, so `default` wins. */
    struct Unrelated { int x; } u = { 0 };
    KOAN_EQ_STR(__STR, TYPE_NAME(u));
}
...
KOAN(the_type_is_the_expressions_type)
{
    ...
    KOAN_EQ_STR(__STR, TYPE_NAME(c));
    KOAN_EQ_STR(__STR, TYPE_NAME(c + 0));

    /* An unsuffixed floating literal is double, not float. */
    KOAN_EQ_STR(__STR, TYPE_NAME(1.5));
    KOAN_EQ_STR(__STR, TYPE_NAME(1.5f));

    /* A string literal is char[N], which decays to char * here. */
    KOAN_EQ_STR(__STR, TYPE_NAME("text"));

    /* An unsuffixed integer literal is int; the suffix changes that. */
    KOAN_EQ_STR(__STR, TYPE_NAME(42));
    KOAN_EQ_STR(__STR, TYPE_NAME(42u));
    KOAN_EQ_STR(__STR, TYPE_NAME(42L));
}
...
KOAN(only_the_selected_branch_is_compiled)
{
    ...
    KOAN_EQ_DBL(__DBL, ABS(-2.5), 1e-12);
...
KOAN(qualifiers_make_distinct_types)
{
    ...
    KOAN_EQ_INT(__, IS_POINTER_TO_INT(p));
    KOAN_EQ_INT(__, IS_POINTER_TO_INT(cp));
    KOAN_EQ_INT(__, IS_POINTER_TO_INT(&d));

    /* An array of int decays to int *, so it matches. */
    int arr[4] = {};
    KOAN_EQ_INT(__, IS_POINTER_TO_INT(arr));
}
...
{
    KOAN_EQ_DBL(__DBL, SQUARE_ROOT(9.0), 1e-12);
    KOAN_EQ_DBL(__DBL, SQUARE_ROOT(4.0f), 1e-6);

    /* The float branch really did use sqrtf: its result is a float, so it
     * carries float precision, not double. */
    KOAN_EQ_SZ(__SZ, sizeof(SQUARE_ROOT(2.0f)));
    KOAN_EQ_SZ(__SZ, sizeof(SQUARE_ROOT(2.0)));
}
...
KOAN(assembling_a_type_safe_dynamic_value)
{
    ...
    KOAN_EQ_INT(__, KIND_OF(1));
    KOAN_EQ_INT(__, KIND_OF(1.0));
    KOAN_EQ_INT(__, KIND_OF("x"));

    Value a = VALUE(42);
    Value b = VALUE(2.5);
    Value c = VALUE("koan");

    KOAN_EQ_INT(__, a.kind);
    KOAN_EQ_STR(__STR, value_render(a, buf, sizeof buf));
    KOAN_EQ_STR(__STR, value_render(b, buf, sizeof buf));
    KOAN_EQ_STR(__STR, value_render(c, buf, sizeof buf));

    /* The X-macro list also produced the count, so adding a kind updates
     * the enum and the count together. */
    KOAN_EQ_INT(__, V_KIND_COUNT);
