...
KOAN(masks_select_bits)
{
    ...
    KOAN_EQ_INT(__, flags & (1u << 2));   /* isolate bit 2 */
    KOAN_EQ_INT(__, (flags >> 2) & 1u);             /* test bit 2 */
    KOAN_EQ_INT(__, (flags >> 3) & 1u);             /* bit 3 is clear */

    /* n low bits set. Note the guard: shifting by the full width is UB. */
    KOAN_EQ_INT(__, (1u << 4) - 1u);
    KOAN_EQ_INT(__, flags & ((1u << 4) - 1u) & 0b1111);
}
...
{
    KOAN_EQ_INT(__, (int)stdc_count_ones(0b1111u));
    KOAN_EQ_INT(__, (int)stdc_count_ones(0b1011'0000u));
    KOAN_EQ_INT(__, (int)stdc_count_ones(0u));

    KOAN_EQ_INT(__, (int)stdc_bit_width(0b1111u));
    KOAN_EQ_INT(__, (int)stdc_bit_width(1u));
    KOAN_EQ_INT(__, (int)stdc_bit_width(0u));
    KOAN_EQ_INT(__, (int)stdc_bit_width(255u));
    KOAN_EQ_INT(__, (int)stdc_bit_width(256u));
}
...
{
    KOAN_EQ_INT(__, (int)stdc_trailing_zeros(0b0001u));
    KOAN_EQ_INT(__, (int)stdc_trailing_zeros(0b1000u));
    KOAN_EQ_INT(__, (int)stdc_trailing_zeros(0b0110u));

    /* log2 of a power of two, exactly and without floating point. */
    KOAN_EQ_INT(__, (int)stdc_trailing_zeros(1024u));
...
{
    KOAN_EQ_INT(__, stdc_has_single_bit(16u));
    KOAN_EQ_INT(__, stdc_has_single_bit(17u));
    KOAN_EQ_INT(__, stdc_has_single_bit(0u));

    /* The identity behind it: clearing the lowest set bit leaves zero. */
    unsigned p = 16;
    KOAN_EQ_INT(__, p & (p - 1));

    KOAN_EQ_INT(__, (int)stdc_bit_floor(31u));
    KOAN_EQ_INT(__, (int)stdc_bit_ceil(31u));
    KOAN_EQ_INT(__, (int)stdc_bit_floor(32u));
    KOAN_EQ_INT(__, (int)stdc_bit_ceil(32u));
}
...
KOAN(assembling_a_bitset)
{
    ...
    KOAN_EQ_SZ(__SZ, BITSET_WORDS);
    KOAN_EQ_SZ(__SZ, sizeof b);        /* 256 bits in 32 bytes */
    KOAN_EQ_SZ(__SZ, bs_count(&b));
    KOAN_EQ_SZ(__SZ, bs_first(&b));   /* empty */
    ...
    KOAN_EQ_SZ(__SZ, bs_count(&b));
    KOAN_EQ_INT(__, bs_test(&b, 64));
    KOAN_EQ_INT(__, bs_test(&b, 65));
    KOAN_EQ_SZ(__SZ, bs_first(&b));

    /* Bit 63 and bit 64 live in different words — the boundary that index
     * arithmetic must get right. */
    KOAN_EQ_INT(__, b.word[0] != 0);
    KOAN_EQ_INT(__, b.word[1] != 0);
    KOAN_EQ_INT(__, b.word[2] == 0);
