...
{
    KOAN_EQ_SZ(__SZ, alignof(char));
    KOAN_EQ_SZ(__SZ, alignof(int32_t));
    KOAN_EQ_SZ(__SZ, alignof(double));

    /* A struct's alignment is the strictest of its members. */
    struct Mixed { char c; double d; };
    KOAN_EQ_SZ(__SZ, alignof(struct Mixed));

    /* Which, with padding, is why its size is a multiple of that alignment. */
    KOAN_EQ_SZ(__SZ, sizeof(struct Mixed));
    KOAN_EQ_SZ(__SZ, sizeof(struct Mixed) % alignof(struct Mixed));
}
...
KOAN(alignas_raises_alignment)
{
    ...
    KOAN_EQ_SZ(__SZ, (uintptr_t)cache_line % 64);
    KOAN_EQ_SZ(__SZ, (uintptr_t)vector % 16);
    ...
    KOAN_EQ_SZ(__SZ, alignof(unsigned char));
}
...
KOAN(malloc_guarantees_only_fundamental_alignment)
{
    ...
    KOAN_EQ_SZ(__SZ, (uintptr_t)ordinary % alignof(max_align_t));
    free(ordinary);

    void *wide = aligned_alloc(64, 128);    /* size must be a multiple of 64 */
    KOAN_TRUE(wide != nullptr);
    KOAN_EQ_SZ(__SZ, (uintptr_t)wide % 64);
    free(wide);
}
...
{
    KOAN_EQ_SZ(__SZ, align_up(1, 8));
    KOAN_EQ_SZ(__SZ, align_up(8, 8));
    KOAN_EQ_SZ(__SZ, align_up(9, 8));
    KOAN_EQ_SZ(__SZ, align_up(0, 8));

    /* The same expression, written the slow way, agrees. */
    KOAN_EQ_SZ(__SZ, ((9 + 8 - 1) / 8) * 8);
}
...
KOAN(restrict_promises_no_overlap)
{
    ...
    KOAN_EQ_INT(__, out[0]);
    KOAN_EQ_INT(__, out[3]);
...
KOAN(volatile_forbids_caching_a_read)
{
    ...
    KOAN_EQ_SZ(__SZ, sizeof(volatile int));
}
...
KOAN(assembling_a_bump_allocator)
{
    ...
    KOAN_EQ_SZ(__SZ, b.used);

    double *d = BUMP_NEW(&b, double);
    KOAN_TRUE(d != nullptr);
    KOAN_EQ_SZ(__SZ, (uintptr_t)d % alignof(double));
