...
KOAN(macros_substitute_tokens_not_values)
{
    ...
    KOAN_EQ_INT(__, BAD_DOUBLE(1 + 1));

    /* GOOD_DOUBLE(1 + 1) becomes ((1 + 1) * 2), which is 4. */
    KOAN_EQ_INT(__, GOOD_DOUBLE(1 + 1));
}
...
KOAN(macro_arguments_are_evaluated_more_than_once)
{
    ...
    i = 0;
    KOAN_EQ_INT(__, safe_max(values[i++], 5));
    KOAN_EQ_INT(__, i);
}
...
{
    KOAN_EQ_STR(__STR, STRINGIZE(1 + 1));
    ...
    KOAN_EQ_STR(__STR, STRINGIZE(WIDTH));
    KOAN_EQ_STR(__STR, EXPAND_THEN(WIDTH));

    int counter_1 = 42;
    KOAN_EQ_INT(__, CONCAT(counter_, 1));
}
...
KOAN(assembling_an_x_macro_table)
{
    ...
    KOAN_EQ_INT(__, HTTP_OK);
    KOAN_EQ_INT(__, HTTP_NOT_FOUND);
    KOAN_EQ_INT(__, HTTP_TEAPOT);

    /* So did the table, and so did its length. */
    KOAN_EQ_SZ(__SZ, STATUS_COUNT);
    KOAN_EQ_STR(__STR, status_text(404));
    KOAN_EQ_STR(__STR, status_text(HTTP_TEAPOT));
    KOAN_EQ_STR(__STR, status_text(999));
