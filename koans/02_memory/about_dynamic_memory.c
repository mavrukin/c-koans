...
KOAN(malloc_gives_bytes_calloc_gives_zeros)
{
    ...
    KOAN_EQ_INT(__, raw[3]);
    free(raw);

    int *zeroed = calloc(4, sizeof *zeroed);
    KOAN_TRUE(zeroed != nullptr);
    KOAN_EQ_INT(__, zeroed[0] + zeroed[1] + zeroed[2] + zeroed[3]);
    free(zeroed);
}
...
{
    KOAN_EQ_INT(__, product_overflows(1000, sizeof(int)));
    KOAN_EQ_INT(__, product_overflows(SIZE_MAX / 2, SIZE_MAX / 2));

    /* The wrap it prevents: this product is small, and a lie. */
    size_t n = SIZE_MAX / 2 + 1;
    KOAN_EQ_SZ(__SZ, n * 2);
    KOAN_EQ_INT(__, product_overflows(n, 2));

    /* Zero is the edge case the guard must not divide by. */
    KOAN_EQ_INT(__, product_overflows(SIZE_MAX, 0));
}
...
KOAN(error_paths_must_free_too)
{
    ...
    KOAN_EQ_INT(__, record_init(&r, "ada", 3));
    KOAN_EQ_STR(__STR, r.name);
    KOAN_EQ_INT(__, r.scores[2]);
