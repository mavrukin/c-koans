...
{
    KOAN_EQ_INT(__, tu_shared_counter);

    /* Modified through the other file's function... */
    KOAN_EQ_INT(__, tu_bump_shared());
    KOAN_EQ_INT(__, tu_shared_counter);
...
{
    KOAN_EQ_INT(__, tu_private_value);
    KOAN_EQ_INT(__, tu_read_private());
...
KOAN(assembling_a_module_boundary)
{
    ...
    KOAN_EQ_INT(__, second.ticket - first.ticket);
    KOAN_TRUE(first.ticket > 0);

    /* The shared counter advances once per issue. */
    KOAN_EQ_INT(__, first.shared_at_issue);
    KOAN_EQ_INT(__, second.shared_at_issue);
    KOAN_EQ_INT(__, tu_shared_counter);

    KOAN_EQ_STR(__STR, second.label);
