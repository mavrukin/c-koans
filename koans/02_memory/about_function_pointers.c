...
KOAN(function_names_decay_to_pointers)
{
    ...
    KOAN_EQ_INT(__, op(3, 4));
    KOAN_EQ_INT(__, (*op)(3, 4));   /* the old spelling; identical */
...
KOAN(callbacks_make_algorithms_generic)
{
    ...
    KOAN_EQ_INT(__, reduce(v, 4, 0, add));
    KOAN_EQ_INT(__, reduce(v, 4, 1, mul));
}
...
KOAN(sorting_strings_needs_a_pointer_to_a_pointer)
{
    ...
    KOAN_EQ_STR(__STR, words[0]);
    KOAN_EQ_STR(__STR, words[1]);
    KOAN_EQ_STR(__STR, words[2]);
}
...
KOAN(bsearch_requires_the_same_ordering)
{
    ...
    KOAN_EQ_INT(__, *found);
    KOAN_EQ_SZ(__SZ, (size_t)(found - v));
...
KOAN(void_user_data_substitutes_for_closures)
{
    ...
    KOAN_EQ_SZ(__SZ, count_if(v, 5, greater_than, &threshold));
...
KOAN(assembling_a_vtable)
{
    ...
    KOAN_EQ_INT(__, total_area);
    KOAN_EQ_INT(__, shapes[0].ops->area(shapes[0].data));
    KOAN_EQ_INT(__, shapes[1].ops->perimeter(shapes[1].data));
    KOAN_EQ_STR(__STR, shapes[1].ops->name);
}
