...
KOAN(network_byte_order_is_big_endian)
{
    ...
    KOAN_EQ_INT(__, inet_pton(AF_INET, "not.an.address", &addr));
}
...
KOAN(a_stream_socket_preserves_order_not_boundaries)
{
    ...
    char buf[16] = {};
    KOAN_EQ_INT(__, (int)read_full(served, buf, 9));
    KOAN_EQ_STR(__STR, buf);

    /* After the peer shuts down writing, read returns 0: end of stream. */
    KOAN_EQ_INT(__, (int)read(served, buf, sizeof buf));
...
KOAN(a_protocol_must_supply_its_own_framing)
{
    ...
    KOAN_EQ_INT(__, send_framed(client, "first message"));
    KOAN_EQ_INT(__, send_framed(client, "second"));

    char buf[64];
    KOAN_EQ_INT(__, recv_framed(served, buf, sizeof buf));
    KOAN_EQ_STR(__STR, buf);
...
{
    int sv[2];
    KOAN_EQ_INT(__, socketpair(AF_UNIX, SOCK_STREAM, 0, sv));

    /* Both directions work on both descriptors. */
    write_full(sv[0], "ping", 4);
    char buf[8] = {};
    KOAN_EQ_INT(__, (int)read_full(sv[1], buf, 4));
    KOAN_EQ_STR(__STR, buf);

    memset(buf, 0, sizeof buf);
    write_full(sv[1], "pong", 4);
    KOAN_EQ_INT(__, (int)read_full(sv[0], buf, 4));
    KOAN_EQ_STR(__STR, buf);
...
KOAN(assembling_a_request_response_server)
{
    ...
    KOAN_EQ_INT(__, send_framed(client, "hello"));
    KOAN_EQ_INT(__, recv_framed(client, reply, sizeof reply));
    KOAN_EQ_STR(__STR, reply);

    /* The connection stays open for a second exchange. */
    KOAN_EQ_INT(__, send_framed(client, "again"));
    KOAN_EQ_INT(__, recv_framed(client, reply, sizeof reply));
    KOAN_EQ_STR(__STR, reply);
    ...
    KOAN_TRUE(second >= 0);
    KOAN_EQ_INT(__, send_framed(second, "still up"));
    KOAN_EQ_INT(__, recv_framed(second, reply, sizeof reply));
    KOAN_EQ_STR(__STR, reply);
    close(second);
